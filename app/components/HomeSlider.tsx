...
const setupEffectFolders = () => {
  ...
  effectFolders.glass.addBinding(SLIDER_CONFIG.settings, 'glassChromaticAberration', {
  ...
  effectFolders.plasma.addBinding(SLIDER_CONFIG.settings, 'plasmaEnergyIntensity', {
    label: 'Energy Intensity',
    min: 0.0,
    max: 1.0,
    step: 0.05,
  });
  effectFolders.plasma.addBinding(SLIDER_CONFIG.settings, 'plasmaContrastBoost', {
  ...
  effectFolders.timeshift.addBinding(SLIDER_CONFIG.settings, 'timeshiftChromatic', {
    label: 'Chromatic Glitch',
    min: 0.0,
    max: 3.0,
    step: 0.1,
  });
  effectFolders.timeshift.addBinding(SLIDER_CONFIG.settings, 'timeshiftTurbulence', {
    label: 'Turbulence',
    min: 0.3,
    max: 3.0,
    step: 0.1,
...
const handleEffectChange = newEffect => {
  ...
  const presetsFolder = pane.children.find(child => child.title === 'Effect Presets');
  if (presetsFolder) {
    // Remove old preset binding
    const oldBinding = presetsFolder.children.find(child => child.key === 'currentEffectPreset');
    if (oldBinding) {
...
const applyEffectPreset = (effectName, presetName) => {
  if (SLIDER_CONFIG.effectPresets[effectName] && SLIDER_CONFIG.effectPresets[effectName][presetName]) {
    isApplyingPreset = true;
    // Apply the preset settings
    Object.assign(SLIDER_CONFIG.settings, SLIDER_CONFIG.effectPresets[effectName][presetName]);
    updateShaderUniforms();
    pane.refresh();
    setTimeout(() => {
